...
function loadStatsPage() {
	...
	for(var i=1; row=table.rows[i]; i++) {
		for(var home=2; home=row.cells[home]; home++) {
			var homeScore = home.innerHTML;
		}
		for(var away=3; away=row.cells[away]; away++) {
			var awayScore = away.innerHTML;
		}
...
function loadPlayersPage() {
	for(let i=0; i<players.length; i++) {
		...
		var link = document.createTextNode(players[i].name);
		a.appendChild(link);
		a.href = "#";
		a.onclick = function() {
			switchPlayers(i);
...
function switchPlayers(playerNum) {
	...
	games = players[playerNum].games_played;
	document.getElementById("avg_p_yards").innerHTML = Math.round(players[playerNum].pass_yards/games);
	document.getElementById("avg_r_yards").innerHTML = Math.round(players[playerNum].rushing_yards/games);
	document.getElementById("avg_rec_yards").innerHTML = Math.round(players[playerNum].receiving_yards/games);
}
